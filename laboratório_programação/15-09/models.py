class UsuarioModel:
    ...
    def salvar(self, nome, email):
        ...
        self._next_id+=1
        return novo_usuario
    def deletar(self, nome_remove, email_remove, novo_usuario):
        remover_usuario = {'id': self._next_id, 'nome' :nome_remove, 'email': email_remove}
        if remover_usuario == novo_usuario:
            
            self._usuarios.splice(remover_usuario)
            self._next_id-=1
